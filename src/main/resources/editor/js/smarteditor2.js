...
nhn.W3CDOMRange = jindo.$Class({
    ...
    cloneContents: function () {
        ...
        oClonedContainers = this._splitTextEndNodes({
            oStartContainer: oClonedContainers.oStartContainer, iStartOffset: this.startOffset,
...
//		this.commonAncestorContainer = this._getCommonAncestorContainer(this.startContainer, this.endContainer);
    ...
    _splitTextEndNodesOfTheRange: function () {
        var oEndPoints = this._splitTextEndNodes({
            oStartContainer: this.startContainer, iStartOffset: this.startOffset,
...
nhn.husky.SE_PasteHandler = jindo.$Class({
    ...
    _filterPastedContents: function (sOriginalContent, bUseTableFilter) {
        ...
        var _widthAttribute = "", _heightAttribute = "", _widthStyle = "", _heightStyle = "", _nWidth = "",
            _nHeight = "",
...
//		this.oApp.exec("REGISTER_UI_EVENT", ["styleRemover", "click", "EXECCOMMAND", ["RemoveFormat", false, false]]);
    ...
    _fixAlign: function (sAlign) {
        ...
        var removeTableAlign = !this.oNavigator.ie ? function () {
        } : function (elNode) {
...
nhn.husky.SE2M_ColorPalette = jindo.$Class({
    ...
    $ON_EVENT_MOUSEOVER_COLOR_PALETTE: function (oEvent) {
        ...
        if (!elHovered || !elHovered.nodeType || elHovered.nodeType == 9) {
            return;
        }
        if (elHovered.className == "" || (!elHovered.className) || typeof (elHovered.className) == 'undefined') {
            jindo.$Element(elHovered).addClass("hover");
        }
    },
    ...
    $ON_EVENT_MOUSEOUT_COLOR_PALETTE: function (oEvent) {
        ...
        if (elHovered.className == "hover") {
            jindo.$Element(elHovered).removeClass("hover");
        }
    },
...
nhn.ColorPicker = jindo.$Class({
    ...
    hsv: function (hsv) {
        ...
        this.fireEvent("colorchange", {
            ...
            currentElement: this,
            rgbColor: rgb,
            hexColor: "#" + this._rgb2hex(rgb.r, rgb.g, rgb.b),
            hsvColor: hsv
        });
    },
    paint: function () {
...
nhn.husky.LazyLoader = jindo.$Class({
    ...
    $LOCAL_BEFORE_ALL: function (sMsgHandler, aParams) {
        ...
        if (htCurMsgInfo.nLoadingStatus == 1) {
            return true;
        }
...
nhn.husky.HuskyCore.mixin(nhn.husky.SE2M_Quote, {
    ...
    _unwrapBlock: function (tag) {
        ...
        while (elCommonAncestor && elCommonAncestor.tagName != tag) {
        ...
        while (elCommonAncestor.firstChild) {
            elCommonAncestor.parentNode.insertBefore(elCommonAncestor.firstChild, elCommonAncestor);
        }
...
//		var sBookmarkID = oSelection.placeStringBookmark();
        ...
        while (oNode && oNode != elCommonAncestor && oNode.parentNode != elCommonAncestor) {
            oNode = oNode.parentNode;
        }
...
nhn.husky.HuskyCore.mixin(nhn.husky.SE2M_SCharacter, {
    ...
    _drawSCharList: function (i) {
        ...
        for (var ii = 0; ii < len; ii++) {
            ...
            aLI[ii].onmouseover = function () {
                this.className = 'hover'
            };
            aLI[ii].onmouseout = function () {
                this.className = ''
            };
...
//		this.oSelection = this.oApp.getSelection();
    ...
    $ON_TABLE_SET_ROW_NUM: function (nRows, nRowDiff) {
        ...
        if (nRows < this.nMinRows) {
            nRows = this.nMinRows;
        }
        if (nRows > this.nMaxRows) {
            nRows = this.nMaxRows;
        }
    ...
    $ON_TABLE_SET_COLUMN_NUM: function (nColumns, nColumnDiff) {
        ...
        if (nColumns < this.nMinColumns) {
            nColumns = this.nMinColumns;
        }
        if (nColumns > this.nMaxColumns) {
    ...
    $ON_TABLE_SET_BORDER_SIZE: function (nBorderWidth, nBorderWidthDiff) {
        ...
        if (nBorderWidth < this.nMinBorderWidth) {
            nBorderWidth = this.nMinBorderWidth;
        }
        if (nBorderWidth > this.nMaxBorderWidth) {
            nBorderWidth = this.nMaxBorderWidth;
        }
    ...
    _doGetTableString: function (nColumns, nRows, nBorderSize, sBorderColor, sBGColor, nBorderStyleIdx) {
        ...
        var sTableStyle = this.aTableStyleByBorder[nBorderStyleIdx].replace(/#BorderSize#/g, this.nBorderSize).replace(/#BorderSize\*([0-9]+)#/g, function (sAll, s1) {
            return nBorderSize * parseInt(s1, 10);
        }).replace(/#BorderSize\+([0-9]+)#/g, function (sAll, s1) {
            return nBorderSize + parseInt(s1, 10);
        }).replace("#BorderColor#", this.sBorderColor).replace("#BGColor#", this.sBGColor);
        var sTDStyle = this.aTDStyleByBorder[nBorderStyleIdx].replace(/#BorderSize#/g, this.nBorderSize).replace(/#BorderSize\*([0-9]+)#/g, function (sAll, s1) {
            return nBorderSize * parseInt(s1, 10);
        }).replace(/#BorderSize\+([0-9]+)#/g, function (sAll, s1) {
            return nBorderSize + parseInt(s1, 10);
        }).replace("#BorderColor#", this.sBorderColor).replace("#BGColor#", this.sBGColor);
...
//		this.oApp.registerBrowserEvent(this.elPanelReviewBtnArea, "click", "SAVE_QE_MY_REVIEW_ITEM"); //My리뷰 버튼 레이어
    ...
    $ON_TABLE_QE_SELECT_TEMPLATE: function (weEvent) {
        ...
        this.oApp.exec("RECORD_UNDO_ACTION", ["CHANGE_TABLE_STYLE", {
            elSaveTarget: elSaveTarget,
            sSaveTarget: sSaveTarget,
        ...
        switch (this.nStatus) {
			...
			if(elTD && elTD.tagName == "TD"){*/
                ...
                if (elTD && this._rxCellNames.test(elTD.tagName)) {
                    ...
                    if (!jindo.$Element(elTBL).hasClass(this._sSETblClass) && !jindo.$Element(elTBL).hasClass(this._sSEReviewTblClass)) {
                        return;
                    }
...
//			jindo.$Element(aCellInfo[0]).css("height", aCellInfo[2]);
    ...
    _isOnBorder: function (wevE) {
        ...
        if (!this._rxCellNames.test(wevE.element.tagName)) {
            return false;
        }
        // --[SMARTEDITORSUS-1672]
    ...
    _getAllAffectedCells: function (basis_x, basis_y, iResizeMode, oTable) {
        ...
        if (iResizeMode == 1) {
            for (var y = 0; y < iTblY; y++) {
                if (aCellsBefore.length > 0 && aCellsBefore[aCellsBefore.length - 1] == oTbl[basis_x][y]) {
                    continue;
                }
            ...
            if (oTbl.length > basis_x + 1) {
                for (var y = 0; y < iTblY; y++) {
                    if (aCellsAfter.length > 0 && aCellsAfter[aCellsAfter.length - 1] == oTbl[basis_x + 1][y]) {
                        continue;
                    }
            ...
            htResult = {
                aCellsBefore: aCellsBefore,
                aCellsAfter: aCellsAfter,
                nMinBefore: nMinBefore,
                nMinAfter: nMinAfter
            };
        } else {
            for (var x = 0; x < iTblX; x++) {
                if (aCellsBefore.length > 0 && aCellsBefore[aCellsBefore.length - 1] == oTbl[x][basis_y]) {
                    continue;
                }
    ...
    _updateSelection: function () {
        this.aSelectedCells = jindo.$A(this.aSelectedCells).filter(function (v) {
            return (v.parentNode !== null && v.parentNode.parentNode !== null);
        }).$value();
    },
...
*/
    ...
    _getNextCell: function (elCell) {
        while (elCell) {
            ...
            if (elCell && elCell.tagName && elCell.tagName.match(/^TD|TH$/)) {
                return elCell;
            }
        }
...
*/
        ...
        for (var i = 0; i < aTD_FirstRow.length; i++) {
            ...
            if (!elTmp.tagName || !elTmp.tagName.match(/^TD|TH$/)) {
                continue;
            }
        ...
        for (var y = 0; y < nTblY; y++) {
            ...
            if (!elCell.tagName || !elCell.tagName.match(/^TD|TH$/)) {
        ...
        for (var x in htProperties) {
            if (htProperties.hasOwnProperty(x)) {
                if (bClearStyle) {
                    ...
                    elTmp = document.createElement(oTarget.tagName);
                    elTmp.style[x] = "";
                    if (!oTarget[x] || oTarget.style[x] == elTmp.style[x]) {
                        oTarget.setAttribute(x, htProperties[x]);
                    }
                }
            }
        }
        ...
        for (var x in htProperties) {
            if (htProperties.hasOwnProperty(x)) {
                if (bClearStyle) {
                    ...
                    elTmp = document.createElement(oTarget.tagName);
                    elTmp.style[x] = "";
                    if (!oTarget.style[x] || oTarget.style[x] == elTmp.style[x] || x.match(/^border/)) {
                        oTarget.style[x] = htProperties[x];
                    }
                }
            }
        }
...
nhn.husky.HuskyCore.mixin(nhn.husky.SE2M_Hyperlink, {
    ...
    $ON_APPLY_HYPERLINK: function () {
        ...
        if (oAgent.ie) {
            sBlank = "<span style=\"text-decoration:none;\">&nbsp;</span>";
        }
...
nhn.husky.HuskyCore.mixin(nhn.husky.SE2M_LineHeightWithLayerUI, {
    ...
    _assignHTMLObjects: function (elAppContainer) {
        ...
        this.aLIOptions = jindo.$A(jindo.$$("LI", this.oDropdownLayer)).filter(function (v, i, a) {
            return (v.firstChild !== null);
        })._array;
    ...
    $ON_LINEHEIGHT_LAYER_SHOWN: function () {
        ...
        if (nLineHeight != null && nLineHeight !== 0) {
            ...
            if (elLi) {
                jindo.$Element(elLi.firstChild).addClass("active");
            }
        } else {
            this.oInput.value = "";
        }
    ...
    _getMatchingLI: function (sValue) {
        ...
        for (var i = 0; i < this.aLIOptions.length; i++) {
            ...
            if (this._getLineHeightFromLI(elLi).toLowerCase() == sValue) {
                return elLi;
            }
        }
